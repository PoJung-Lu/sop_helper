...
APP_TITLE = "SOP_Helper MVP v0.9.9"
...
class MainWindow(QMainWindow):
    ...
    def update_canvas_marks(self):
        ...
        for a in self.annotations:
            ...
            size_ratio = 0.7 if digit_count <= 1 else (0.5 if digit_count == 2 else 0.38)
            font.setPointSizeF(max(6, r*size_ratio)); text.setFont(font)
    ...
    def render_composite(self):
        ...
        for a in self.annotations:
            ...
            digit_count = len(sequence_text)
            seq_font_size = max(8, int(r * (1.1 if digit_count <= 1 else (0.85 if digit_count == 2 else 0.68))))
            seq_font = get_windows_font(seq_font_size)
            bbox = draw.textbbox((0, 0), sequence_text, font=seq_font)
            tw, th = bbox[2]-bbox[0], bbox[3]-bbox[1]
            draw.text((x-tw/2, y-th/2-bbox[1]), sequence_text, fill=MARKER_COLOR, font=seq_font)
    ...
    def save_project(self):
        ...
        if path:
            data = {"project_version":"0.9.9", "source_image":self.image_path, "annotations":[asdict(a) for a in self.annotations], "components":[asdict(c) for c in self.components]}
