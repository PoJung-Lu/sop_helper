...
class AnnotationDialog(QDialog):
    def __init__(self, component: Component, sequence: int, parent=None):
        ...
        self.torque.setValue(component.required_torque or 0)
        form.addRow("本次扭力", self.torque)

        self.unit = QLineEdit(component.torque_unit)
    ...
    def values(self):
        return self.torque.value(), self.unit.text().strip(), self.sequence.value()
...
class MainWindow(QMainWindow):
    ...
    def render_composite(self):
        ...
        for annotation in self.annotations:
            ...
            draw.ellipse((x-r, y-r, x+r, y+r), outline=(220, 0, 0, 255), width=max(3, r // 5))
            draw.text((x + r + 5, y - r), f"{annotation.sequence}. {component.part_name}", fill=(220, 0, 0, 255))
            torque_text = f"{annotation.torque:g} {annotation.torque_unit}" if annotation.torque is not None else "Torque N/A"
            draw.text((x + r + 5, y + 3), torque_text, fill=(180, 0, 0, 255))
        return base
    ...
    def save_pptx(self):
        ...
        if not path:
            return
        try:
            export_pptx(path, self.image_path, self.components, self.annotations)
...
def validate_components(components):
    ...
    for i, c in enumerate(components, 1):
        ...
        if c.required_torque is None or not c.torque_unit:
            errors.append(f"{c.part_number} 缺少 required_torque 或 torque_unit")
...
def export_pptx(path, image_path, components, annotations):
    prs = Presentation()
    ...
    for annotation in annotations:
        ...
        run = p.add_run()
        torque = f"{annotation.torque:g} {annotation.torque_unit}" if annotation.torque is not None else "Torque N/A"
        run.text = f"{annotation.sequence}. {component.part_name}\n{torque}"
        run.font.size = Pt(12)
        run.font.color.rgb = RGBColor(190, 0, 0)

        line = slide.shapes.add_connector(
            1, tx, ty + Inches(0.22), x, y
        )
