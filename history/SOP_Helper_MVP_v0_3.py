...
APP_TITLE = "SOP_Helper MVP v0.3"
TORQUE_UNITS = ["kgf-cm", "N-M", "無需扭力"]
NO_TORQUE = "無需扭力"
...
class AnnotationDialog(QDialog):
    def __init__(self, component: Component, sequence: int, parent=None):
        ...
        form.addRow(
            "料件",
            QLabel(f"{component.part_name} ({component.part_number})\n尺寸：{component.size}"),
        )
    ...
    def _unit_changed(self, unit: str):
        self.torque.setEnabled(unit != NO_TORQUE)
        if unit == NO_TORQUE:
...
class MainWindow(QMainWindow):
    ...
    def load_spec(self):
        ...
        try:
            ...
            for index, component in enumerate(self.components):
                ...
                item = QListWidgetItem(f"{component.part_name} | {component.part_number} | {torque}")
                item.setData(Qt.ItemDataRole.UserRole, index)
                self.component_list.addItem(item)
            msg = f"已載入 {len(self.components)} 筆料件規格"
            if warnings:
                msg += f"（{len(warnings)} 筆提示）"
    ...
    def update_canvas_marks(self):
        ...
        for annotation in self.annotations:
            ...
            r = 18
            color = QColor("red")
            ellipse = self.canvas.scene.addEllipse(x - r, y - r, 2 * r, 2 * r, QPen(color, 3))
            ellipse.setZValue(2)
    ...
    @staticmethod
    def torque_display(annotation: Annotation) -> str:
        if annotation.torque is None or annotation.torque_unit == NO_TORQUE:
            return ""
        return f"{annotation.torque:g} {annotation.torque_unit}"
    ...
    def render_composite(self):
        ...
        for annotation in self.annotations:
            ...
            torque_text = self.torque_display(annotation)
            if torque_text:
                draw.text((x + r + 5, y + 3), torque_text, fill=color)
        return base
...
def validate_components(components):
    ...
    for i, c in enumerate(components, 1):
        ...
        elif c.torque_min is not None and c.torque_max is not None:
...
def export_pptx(path, image_path, components, annotations, window: MainWindow):
    ...
    for annotation in annotations:
        ...
        circle.line.color.rgb = RGBColor(220, 0, 0)
        ...
        run = p.add_run()
        torque = window.torque_display(annotation)
        run.text = f"{annotation.sequence}. {component.part_name}" + (f"\n{torque}" if torque else "")
        run.font.size = Pt(12)
        run.font.color.rgb = RGBColor(220, 0, 0)
        line = slide.shapes.add_connector(1, tx, ty + Inches(0.22), x, y)
        line.line.color.rgb = RGBColor(220, 0, 0)
