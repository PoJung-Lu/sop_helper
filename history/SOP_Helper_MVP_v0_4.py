...
class AnnotationDialog(QDialog):
    ...
    def on_unit_changed(self, unit):
        is_numeric = unit in {"kgf-cm", "N-M"}
        self.torque.setEnabled(is_numeric)
        if unit == NO_TORQUE:
    ...
    def values(self):
        ...
        if unit == NO_TORQUE or unit == LOCK_ONLY or self.torque.value() == 0:
            return None, unit, self.sequence.value()
...
class MainWindow(QMainWindow):
    ...
    def build_left_panel(self):
        ...
        layout.addWidget(b)
        b = QPushButton("載入 Excel / YAML")
        b.clicked.connect(self.load_spec)
        layout.addWidget(b)
        layout.addWidget(QLabel("可用料件（雙擊後在圖片上新增標記）"))
        layout.addWidget(self.component_list, 1)
        b = QPushButton("使用選取料件新增標記")
        b.clicked.connect(self.add_selected_component)
        layout.addWidget(b)
        b = QPushButton("刪除最後一個標記")
        b.clicked.connect(self.delete_last_annotation)
        layout.addWidget(b)
        return box
    ...
    def build_right_panel(self):
        ...
        for text, slot in [
            ("更新輸出預覽", self.update_preview),
            ("複製合成圖片", self.copy_composite_image),
    ...
    def update_canvas_marks(self):
        ...
        for a in self.annotations:
            ...
            text = self.canvas.scene.addSimpleText(str(a.sequence))
    ...
    def torque_display(a):
        if a.torque is None or a.torque_unit in {NO_TORQUE, LOCK_ONLY}:
            return "" if a.torque_unit == NO_TORQUE else LOCK_ONLY
        return f"{a.torque:g} {a.torque_unit}"

    def render_composite(self):
        base = Image.open(self.image_path).convert("RGBA")
        draw = ImageDraw.Draw(base)
        font = get_windows_font(max(16, min(base.size) // 32))
        width, height = base.size
        for a in self.annotations:
            c = self.components[a.component_index]
            x, y = int(a.x * width), int(a.y * height)
            r = max(12, min(width, height) // 45)
            color = (220, 0, 0, 255)
            draw.ellipse((x-r, y-r, x+r, y+r), outline=color, width=max(3, r // 5))
            draw.text((x+r+5, y-r), f"{a.sequence}. {c.part_name}", fill=color, font=font)
            torque = self.torque_display(a)
            if torque:
                draw.text((x+r+5, y+3), torque, fill=color, font=font)
        return base
    ...
    def save_project(self):
        ...
        if path:
            data = {"project_version": "0.4", "source_image": self.image_path, "annotations": [asdict(a) for a in self.annotations], "components": [asdict(c) for c in self.components]}
    ...
    def copy_composite_image(self):
        ...
        try:
            ...
            output = BytesIO()
            self.render_composite().convert("RGB").save(output, "BMP")
            data = output.getvalue()[14:]
            win32clipboard.OpenClipboard(); win32clipboard.EmptyClipboard()
            win32clipboard.SetClipboardData(win32con.CF_DIB, data); win32clipboard.CloseClipboard()
    ...
    for path in candidates:
        if Path(path).exists():
            try:
                ...
                continue
    raise RuntimeError("找不到可顯示中文的 Windows 字型，請確認 C:\\Windows\\Fonts 中存在 msyh.ttc 或 simhei.ttf")
...
def component_from_dict(row):
    return Component(
        ...
        required_torque=to_float(row.get("required_torque")),
        ...
        torque_min=to_float(row.get("torque_min")), torque_max=to_float(row.get("torque_max")),
...
def validate_components(components):
    ...
    for i, c in enumerate(components, 1):
        ...
        has_target, has_unit = c.required_torque is not None, bool(c.torque_unit)
        if has_target != has_unit:
            errors.append(f"{c.part_number} 的 required_torque 與 torque_unit 必須同時填寫或同時留空")
        elif not has_target:
            warnings.append(f"{c.part_number}：未設定扭力，視為無需扭力元件")
        elif c.torque_min is not None and c.torque_max is not None and not (c.torque_min <= c.required_torque <= c.torque_max):
...
def load_components(path):
    ...
    if path.suffix.lower() == ".xlsx":
        ...
        rows = [dict(zip(headers, values)) for values in sheet.iter_rows(min_row=2, values_only=True) if not all(v is None for v in values)]
    elif path.suffix.lower() in {".yaml", ".yml"}:
        rows = (yaml.safe_load(path.read_text(encoding="utf-8")) or {}).get("components", [])
    else:
        raise ValueError("只支援 .xlsx、.yaml、.yml")
    components = [component_from_dict(row) for row in rows]
    return components, validate_components(components)
...
def export_pptx(path, image_path, components, annotations, window):
    ...
    image_width = Inches(8.5); image_height = image_width * pic.height / pic.width
    for a in annotations:
        c = components[a.component_index]
        x, y = left + int(image_width * a.x), top + int(image_height * a.y)
        radius = Inches(0.12)
        circle = slide.shapes.add_shape(MSO_SHAPE.OVAL, x-radius, y-radius, radius*2, radius*2)
        circle.fill.background(); circle.line.color.rgb = RGBColor(220, 0, 0); circle.line.width = Pt(2.5)
        tx, ty = x + Inches(0.16), y - Inches(0.18)
        textbox = slide.shapes.add_textbox(tx, ty, Inches(2.8), Inches(0.6))
        p = textbox.text_frame.paragraphs[0]; p.alignment = PP_ALIGN.LEFT
        run = p.add_run(); torque = window.torque_display(a)
        run.text = f"{a.sequence}. {c.part_name}" + (f"\n{torque}" if torque else "")
        run.font.size = Pt(12); run.font.color.rgb = RGBColor(220, 0, 0)
        line = slide.shapes.add_connector(1, tx, ty + Inches(0.22), x, y)
        line.line.color.rgb = RGBColor(220, 0, 0); line.line.width = Pt(2)
    prs.save(path)
