...
class MainWindow(QMainWindow):
    ...
    def build_right_panel(self):
        ...
        b = QPushButton("更新輸出預覽"); b.clicked.connect(self.update_preview); layout.addWidget(b)
        b = QPushButton("複製為 PowerPoint 物件（可編輯）")
        b.setStyleSheet("font-weight:bold;")
        b.clicked.connect(self.copy_pptx_shapes)
        layout.addWidget(b)
        b = QPushButton("複製合成圖片（僅供備援）"); b.clicked.connect(self.copy_composite_image); layout.addWidget(b)
        b = QPushButton("另存 PNG"); b.clicked.connect(self.save_png); layout.addWidget(b)
        b = QPushButton("另存可編輯 PPTX"); b.clicked.connect(self.save_pptx); layout.addWidget(b)
        b = QPushButton("另存專案 JSON"); b.clicked.connect(self.save_project); layout.addWidget(b)
        return box
    ...
    def save_project(self):
        ...
        if path:
            data = {"project_version":"0.6", "source_image":self.image_path, "annotations":[asdict(a) for a in self.annotations], "components":[asdict(c) for c in self.components]}
    ...
    def copy_pptx_shapes(self):
        if not self.image_path:
            ...
            return
        if win32com is None:
            QMessageBox.critical(self, "缺少套件", "請安裝 pywin32（python -m pip install pywin32）")
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "暫存 PPTX 建立失敗", str(exc))
            return
        ...
        try:
            ...
            if shape_range.Count == 0:
                raise RuntimeError("投影片中沒有可複製的物件")
            shape_range.Copy()
...
def export_pptx(path, image_path, components, annotations, window):
    ...
    for a in annotations:
        ...
        tf=circle.text_frame; tf.clear(); tf.word_wrap=False
        p=tf.paragraphs[0]; p.alignment=PP_ALIGN.CENTER; run=p.add_run(); run.text=str(a.sequence); run.font.size=Pt(11); run.font.bold=True; run.font.color.rgb=RGBColor(220,0,0)
        tx,ty=x+Inches(.16),y-Inches(.18)
        name_box=slide.shapes.add_textbox(tx,ty,Inches(2.8),Inches(.35)); name_box.text_frame.word_wrap=True
        p=name_box.text_frame.paragraphs[0]; run=p.add_run(); run.text=c.part_name; run.font.size=Pt(12); run.font.color.rgb=RGBColor(0,0,0)
        torque=window.torque_display(a)
        if torque:
            tb=slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE,tx,ty+Inches(.4),Inches(2.0),Inches(.35))
            tb.fill.background(); tb.line.color.rgb=RGBColor(210,0,0); tb.line.width=Pt(0.75)
            p=tb.text_frame.paragraphs[0]; p.alignment=PP_ALIGN.CENTER; run=p.add_run(); run.text=torque; run.font.size=Pt(11); run.font.color.rgb=RGBColor(0,0,0)
