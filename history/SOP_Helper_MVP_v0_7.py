...
class AnnotationDialog(QDialog):
    ...
    def on_unit_changed(self, unit):
        ...
        self.torque.setEnabled(numeric and not self.marker_only.isChecked())
    ...
    def values(self):
        ...
        if self.marker_only.isChecked():
            return None, NO_TORQUE, self.sequence.value(), True
        if unit == NO_TORQUE or self.torque.value() == 0:
            return None, NO_TORQUE, self.sequence.value(), False
        return self.torque.value(), unit, self.sequence.value(), False
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.preview_label.setMinimumWidth(340)
    ...
    def zoom_out(self): self.canvas.set_zoom(self.canvas.zoom_factor * 0.8)
    def zoom_100(self): self.canvas.set_zoom(1.0)
    def zoom_in(self): self.canvas.set_zoom(self.canvas.zoom_factor * 1.25)
    ...
    def add_annotation_at(self, pos, component_index=None):
        ...
        self.annotations.append(Annotation(f"F{len(self.annotations)+1:03d}", component_index, pos.x()/rect.width(), pos.y()/rect.height(), sequence, torque, unit, marker_only))
    ...
    def save_project(self):
        ...
        if path:
            data = {"project_version":"0.7", "source_image":self.image_path, "annotations":[asdict(a) for a in self.annotations], "components":[asdict(c) for c in self.components]}
...
def apply_ppt_font(text_frame):
    text_frame.font.name = WINDOWS_FONT_FAMILY
    for paragraph in text_frame.paragraphs:
        for run in paragraph.runs:
            run.font.name = WINDOWS_FONT_FAMILY
...
def export_pptx(path, image_path, components, annotations, window):
    ...
    for a in annotations:
        ...
        tf=circle.text_frame; tf.clear(); tf.vertical_anchor=MSO_ANCHOR.MIDDLE; p=tf.paragraphs[0]; p.alignment=PP_ALIGN.CENTER; run=p.add_run(); run.text=str(a.sequence); run.font.name=WINDOWS_FONT_FAMILY; run.font.size=Pt(11); run.font.bold=True; run.font.color.rgb=RGBColor(220,0,0); parts.append(circle)
        tx,ty=x+Inches(.16),y-Inches(.18)
        if not a.marker_only:
            name_box=add_textbox(slide,tx,ty,Inches(2.8),Inches(.35),c.part_name,12,RGBColor(0,0,0)); parts.append(name_box)
            torque=window.torque_display(a)
            if torque:
                tb=slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE,tx,ty+Inches(.4),Inches(2.0),Inches(.35)); tb.fill.background(); tb.line.color.rgb=RGBColor(210,0,0); tb.line.width=Pt(0.75); tf=tb.text_frame; tf.clear(); tf.vertical_anchor=MSO_ANCHOR.MIDDLE; p=tf.paragraphs[0]; p.alignment=PP_ALIGN.CENTER; run=p.add_run(); run.text=torque; run.font.name=WINDOWS_FONT_FAMILY; run.font.size=Pt(11); run.font.color.rgb=RGBColor(0,0,0); parts.append(tb)
            line=slide.shapes.add_connector(1,tx,ty+Inches(.18),x,y); line.line.color.rgb=RGBColor(220,0,0); line.line.width=Pt(2); parts.append(line)
        # 透過 PowerPoint COM 進行群組；此處先保留標記元素清單。
    prs.save(path)
