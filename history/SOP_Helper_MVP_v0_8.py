...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.component_list.itemSelectionChanged.connect(self.update_mode_hint)
    ...
    def build_left_panel(self):
        ...
        layout.addWidget(QLabel("可用料件（選取後點擊圖片新增標記；再點一次取消選取）"))
        ...
        b = QPushButton("取消選取料件（切換為瀏覽模式）"); b.clicked.connect(self.component_list.clearSelection); layout.addWidget(b)
    ...
    def build_center_panel(self):
        ...
        layout.addWidget(QLabel("操作提示：Ctrl+滾輪＝以滑鼠位置縮放，中鍵拖曳＝平移，左側清單選取料件後點圖新增標記"))
        return box
    ...
    def update_mode_hint(self):
        if self.has_component_selected():
            ...
            self.mode_hint.setText("目前模式：瀏覽 / 選取（點擊圖片可選取既有標記，拖曳可平移）")

    def has_component_selected(self):
        return self.component_list.currentItem() is not None and self.component_list.currentItem().isSelected()
    ...
    def try_select_marker_at(self, scene_pos, tolerance=22):
        ...
        self.update_canvas_marks()
        if best:
            self.status.setText(f"已選取標記 {best.sequence}（{self.components[best.component_index].part_name}）")
    ...
    def save_project(self):
        ...
        if path:
            data = {"project_version":"0.8", "source_image":self.image_path, "annotations":[asdict(a) for a in self.annotations], "components":[asdict(c) for c in self.components]}
...
def export_pptx(path, image_path, components, annotations, window):
    ...
    for a in annotations:
        ...
        tf=circle.text_frame; tf.clear(); tf.vertical_anchor=MSO_ANCHOR.MIDDLE; p=tf.paragraphs[0]; p.alignment=PP_ALIGN.CENTER; run=p.add_run(); run.text=str(a.sequence); run.font.name=WINDOWS_FONT_FAMILY; run.font.size=Pt(11); run.font.bold=True; run.font.color.rgb=RGBColor(220,0,0)
        tx,ty=x+Inches(.16),y-Inches(.18)
        if not a.marker_only:
            add_textbox(slide,tx,ty,Inches(2.8),Inches(.35),c.part_name,12,RGBColor(0,0,0))
            torque=window.torque_display(a)
            if torque:
                tb=slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE,tx,ty+Inches(.4),Inches(2.0),Inches(.35)); tb.fill.background(); tb.line.color.rgb=RGBColor(210,0,0); tb.line.width=Pt(0.75); tf=tb.text_frame; tf.clear(); tf.vertical_anchor=MSO_ANCHOR.MIDDLE; p=tf.paragraphs[0]; p.alignment=PP_ALIGN.CENTER; run=p.add_run(); run.text=torque; run.font.name=WINDOWS_FONT_FAMILY; run.font.size=Pt(11); run.font.color.rgb=RGBColor(0,0,0)
