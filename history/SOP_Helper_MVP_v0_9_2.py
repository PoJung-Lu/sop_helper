...
APP_TITLE = "SOP_Helper MVP v0.9.2"
...
class MainWindow(QMainWindow):
    ...
    def save_project(self):
        ...
        if path:
            data = {"project_version":"0.9.2", "source_image":self.image_path, "annotations":[asdict(a) for a in self.annotations], "components":[asdict(c) for c in self.components]}
...
def export_pptx(path, image_path, components, annotations, window):
    ...
    for a in annotations:
        ...
        if not a.marker_only:
            ...
            line_start_x,line_start_y=tx,ty+Inches(.18)
            line_end_x,line_end_y=point_toward(line_start_x,line_start_y,x,y,back_off=radius)
            line=slide.shapes.add_connector(1,line_start_x,line_start_y,line_end_x,line_end_y); line.line.color.rgb=RGBColor(220,0,0); line.line.width=Pt(2)
            add_arrowhead(line)
    prs.save(path)
