...
APP_TITLE = "SOP_Helper MVP v0.9.3"
...
class MainWindow(QMainWindow):
    ...
    def build_center_panel(self):
        ...
        layout.addWidget(QLabel("操作提示：可直接把圖片檔案拖曳到畫布載入；Ctrl+滾輪＝以滑鼠位置縮放；中鍵拖曳＝平移"))
        return box
    ...
    def save_project(self):
        ...
        if path:
            data = {"project_version":"0.9.3", "source_image":self.image_path, "annotations":[asdict(a) for a in self.annotations], "components":[asdict(c) for c in self.components]}
...
def export_pptx(path, image_path, components, annotations, window):
    ...
    for a in annotations:
        c=components[a.component_index]; x,y=left+int(image_width*a.x),top+int(image_height*a.y); radius=Inches(.09)
        ...
        tf=circle.text_frame; tf.clear(); tf.vertical_anchor=MSO_ANCHOR.MIDDLE; p=tf.paragraphs[0]; p.alignment=PP_ALIGN.CENTER; run=p.add_run(); run.text=str(a.sequence); run.font.name=WINDOWS_FONT_FAMILY; run.font.size=Pt(9); run.font.bold=True; run.font.color.rgb=RGBColor(220,0,0)
        ...
        if not a.marker_only:
            ...
            if torque:
                ...
                anchor_shape=tb
            line_start_x,line_start_y=point_toward(x,y,anchor_shape.left+anchor_shape.width//2,anchor_shape.top+anchor_shape.height//2,back_off=0)
            line_end_x,line_end_y=point_toward(anchor_shape.left+anchor_shape.width//2,anchor_shape.top+anchor_shape.height//2,x,y,back_off=radius)
            line=slide.shapes.add_connector(1,line_start_x,line_start_y,line_end_x,line_end_y)
            line.line.color.rgb=RGBColor(220,0,0); line.line.width=Pt(2)
            try:
                line.begin_connect(anchor_shape, 0)
                line.end_connect(circle, 0)
            except Exception:
                pass
            add_arrowhead(line)
    prs.save(path)
