...
APP_TITLE = "SOP_Helper MVP v0.9.4"
...
class MainWindow(QMainWindow):
    ...
    def build_center_panel(self):
        ...
        layout.addWidget(QLabel("操作提示：可直接把圖片檔案拖曳到畫布載入；Ctrl+滾輪＝以滑鼠位置縮放；中鍵拖曳＝平移；瀏覽模式下拖曳既有標記可移動位置，雙擊可編輯內容"))
        return box
    ...
    def save_project(self):
        ...
        if path:
            data = {"project_version":"0.9.4", "source_image":self.image_path, "annotations":[asdict(a) for a in self.annotations], "components":[asdict(c) for c in self.components]}
