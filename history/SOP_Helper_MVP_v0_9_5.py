...
APP_TITLE = "SOP_Helper MVP v0.9.5"
...
PPTX_MARKER_RADIUS_RATIO = 0.09 / 8.5
...
class MainWindow(QMainWindow):
    ...
    def update_canvas_marks(self):
        ...
        rect = self.canvas.scene.sceneRect()
        # 標記圓圈半徑採用與 PPTX/PNG 輸出一致的比例（半徑 / 圖片寬度 ≈ 0.09/8.5），
        ...
        for a in self.annotations:
            ...
            font = text.font(); font.setBold(True); font.setPointSizeF(max(7, r*0.7)); text.setFont(font)
    ...
    def render_composite(self):
        ...
        for a in self.annotations:
            ...
            r = max(8, int(width * PPTX_MARKER_RADIUS_RATIO))
    ...
    def save_project(self):
        ...
        if path:
            data = {"project_version":"0.9.5", "source_image":self.image_path, "annotations":[asdict(a) for a in self.annotations], "components":[asdict(c) for c in self.components]}
...
def add_arrowhead(connector_shape):
    # python-pptx 沒有直接的箭頭 API，改寫 XML 的 <a:tailEnd> 節點加上三角形箭頭頭。
    ...
    for a in annotations:
        ...
        if not a.marker_only:
            ...
            if torque:
                ...
                anchor_shape=tb
            # 起點固定在說明方框的左緣中點（連接點索引 1），終點固定在標記圓圈的右緣中點（索引 3），
            # 因為版面配置是「圓圈在左、說明文字在右」，這樣箭頭視覺上才是從方框指向圓圈右側。
